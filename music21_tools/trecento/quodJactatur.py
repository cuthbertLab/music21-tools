# ------------------------------------------------------------------------------
# Name:         music21_tools/trecento/quodJactatur.py
...
#    genericInterval = interval.GenericInterval(diatonicNumber)
    ...
    for n in myStream.flatten().notesAndRests:
...
def findRetrogradeVoices(show=True):
    ...
    for transpose in [1, 2, -2, 3, -3, 4, -4]:
        for invert in [False, True]:
            ...
            if invert is True:
                qj2.invertDiatonic(qj2.flatten().notesAndRests[0], inPlace=True)
            qj2 = reverse(qj2, makeNotation = False)
            qj = stream.Score()
            qj.insert(0, qj2.flatten())
            qj.insert(0, qj1.flatten())
            qjChords = qj.chordify()
            consScore = 0
            totIntervals = 0
            for n in qjChords.flatten().notesAndRests:
            ...
            qj1.flatten().notesAndRests[0].addLyric('Score: ' + str(finalScore))
...
def prepareSolution(triplumTup, ctTup, tenorTup):
    ...
    for transpose, delay, invert, retro in [triplumTup, ctTup, tenorTup]:
        ...
        if idString in cachedParts:
            ...
            if invert is True:
                qjPart.invertDiatonic(qjPart.flatten().notesAndRests[0], inPlace=True)
            if transpose != 1:
