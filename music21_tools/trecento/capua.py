# ------------------------------------------------------------------------------
# Name:         music21_tools/trecento/capua.py
...
def applyCapuaToScore(thisWork):
    ...
    for thisPart in thisWork.parts:
        applyCapuaToStream(thisPart.flatten().notes.stream())
...
def applyCapuaToCadencebookWork(thisWork):
    ...
    >>> bOrigFN = bOrig.asScore().flatten().notes
...
def capuaRuleOne(srcStream):
    ...
    ssn = srcStream.flatten().notesAndRests
...
def clearFicta(srcStream1):
    ...
    for n2 in srcStream1.flatten().notes:
...
def compareThreeFictas(srcStream1, srcStream2):
    ...
    for note1 in srcStream1.notes:
        ...
        if 'pmfcFicta' in note1.editorial:
            pmfcFictaToAccidental(note1)
            _hi = note1.editorial.harmonicInterval
            note1.editorial.harmonicInterval = interval.Interval(_hi.noteStart, _hi.noteEnd)
            if hasattr(note1.editorial.harmonicInterval, 'name'):
...
def compareSrcStreamCapuaToEditor(srcStream1):
    ...
    for note1 in srcStream1.flatten().notesAndRests:
...
def runPiece(pieceNum=331, snipNum=0):  # random default piece...
    ...
    srcStream = pieceObj.snippets[snipNum].parts[0].flatten().notesAndRests
    cmpStream = pieceObj.snippets[snipNum].parts[1].flatten().notesAndRests
    # ignore 3rd voice for now...
    ...
    for i in range(startNumber, endNumber):  # all ballate
        ...
        for thisPolyphonicSnippet in pieceObj.snippets:
            ...
            for thisPart in thisPolyphonicSnippet.parts:
                thisStream = thisPart.flatten().notes
...
class Test(unittest.TestCase):
    ...
    def testRun1(self):
        ...
        if len(cadenceA.parts) >= 2:
            srcStream1 = cadenceA.parts[0].flatten().notes.stream()
            srcStream2 = cadenceA.parts[1].flatten().notes.stream()  # ignore 3rd voice for now...
    ...
    def testCompare1(self):
        ...
        for i in range(232, 349):  # 232-349 is most of Landini PMFC
            ...
            if cadenceA is not None and len(cadenceA.parts) >= 2:
                srcStream1 = cadenceA.parts[0]  # .flatten().notesAndRests
                # srcStream2 = cadenceA.parts[1].flatten().notesAndRests
                # # ignore 3rd voice for now...
