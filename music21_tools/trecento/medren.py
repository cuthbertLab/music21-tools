# ------------------------------------------------------------------------------
# Name:         music21_tools/trecento/medren.py
...
class MensuralClef(clef.Clef):
    ...
    >>> from music21_tools.trecento.medren import MensuralClef
    >>> fclef = MensuralClef('F')
    >>> fclef.line
    3
    >>> fclef.fontString
    '0x5c'
...
class Mensuration(MedievalMeter):
    '''
    ...
    >>> from music21_tools.trecento.medren import Mensuration
    >>> ODot = Mensuration(tempus='perfect', prolation='major')
    >>> ODot.standardSymbol
    'O-dot'
    >>> ODot.fontString
    '0x50'
    ...
    def __str__(self):
        return '<medren.Mensuration %s>' % self.standardSymbol
...
class GeneralMensuralNote(base.Music21Object):
    '''
    The base class object for :class:`medren.MensuralNote` and
    :class:`medren.MensuralRest`. This is arguably the most important mensural object,
    since it is responsible for getting the context and determining the contextual duration of
    objects within both subclasses.
    A :class:`medren.GeneralMensuralNote` object takes a mensural type or its
    ...
    :attr:`medren.GeneralMensuralNote.mensuralType`.
    The duration of a general mensural note can be set and accessed using the property
    :attr:`medren.GeneralMensuralNote.duration`. If the duration of an general
    mensural note cannot be determined from context, it is set to 0. For more specific
    examples of this, please see the :attr:`medren.GeneralMensuralNote.duration`
    documentation.
    ...
    def __repr__(self):
        return '<medren.GeneralMensuralNote %s>' % self.mensuralType
    ...
    def __eq__(self, other):
        ...
        >>> from music21_tools.trecento.medren import GeneralMensuralNote
        >>> from music21 import stream
    ...
    @property
    def mensuralType(self):
        '''
        Name of the mensural length of the general mensural note
        (brevis, longa, etc.):
        ...
        music21_tools.trecento.medren.MedRenException: blah is not a valid
            mensural type or abbreviation
        '''
        return self._mensuralType

    @mensuralType.setter
    def mensuralType(self, mensuralTypeOrAbbr):
        if mensuralTypeOrAbbr in _validMensuralTypes:
            self._mensuralType = mensuralTypeOrAbbr
        elif mensuralTypeOrAbbr in _validMensuralAbbr:
            self.mensuralType = _validMensuralTypes[_validMensuralAbbr.index(mensuralTypeOrAbbr)]
    ...
    def updateDurationFromMensuration(self, mensuration=None, surroundingStream=None):
        '''
        The duration of a :class:`GeneralMensuralNote` object can be accessed
        and set using the :attr:`GeneralMensuralNote.duration` property.
        The duration of a general mensural note is by default 0. If the object's subclass
        is not specified (:class:`MensuralNote` or
        :class:`MensuralRest`), the duration will remain 0 unless set to
        some other value.
        ...
        :meth:`GeneralMensuralNote.updateDurationFromMensuration`` should be called.

        >>> from music21_tools.trecento.notation import Divisione, Punctus
        >>> from music21_tools.trecento import notation
        >>> from music21_tools.trecento.medren import GeneralMensuralNote, MensuralNote
        >>> mn = GeneralMensuralNote('B')
        ...
        >>> for i in range(3):
        ...    s.append(MensuralNote('A', 'SB'))
        >>> s.append(Punctus())
        >>> s.append(MensuralNote('B', 'SB'))
        >>> s.append(MensuralNote('B', 'SB'))
        >>> s.append(Punctus())
        >>> s.append(MensuralNote('A', 'B'))
        >>> for mn in s:
        ...    if isinstance(mn, GeneralMensuralNote):
        ...
        if mOrD is None:
            # No mensuration or divisione in context — duration cannot be inferred.
    ...
    def _getTranslator(self, mensurationOrDivisione=None, surroundingStream=None):
        ...
        if measure and mOrD is not None and isinstance(mOrD, notation.Divisione):
            if index == 0:
                self.lenList = notation.BrevisLengthTranslator(
    ...
    def _determineMensurationOrDivisione(self):
        ...
        >>> from music21_tools.trecento.medren import Mensuration
        >>> from music21_tools.trecento.notation import Divisione
        >>> from music21_tools.trecento.medren import GeneralMensuralNote
        ...
        >>> s_3 = stream.Stream()
        >>> s_3.insert(3, gmn)
        >>> s_2.insert(1, Divisione('.q.'))
        >>> s_1.insert(2, Mensuration('perfect', 'major'))
        >>> s_2.insert(2, s_3)
        >>> s_1.insert(3, s_2)
        >>> gmn._determineMensurationOrDivisione()
        <notation.Divisione .q.>
        '''
        # In music21 v10, getContextByClass takes a single class only, so we
        # search for the common parent of Mensuration and Divisione.
        mOrD = self.getContextByClass(MedievalMeter)
        if mOrD is not None:
            return mOrD
        else:
            return None
    ...
    def _getSurroundingMeasure(self, mensurationOrDivisione=None, activeSite=None):
        ...
        >>> from music21_tools.trecento.medren import GeneralMensuralNote, Ligature, MensuralNote
        >>> from music21_tools.trecento.notation import Divisione, Punctus
        >>> s_1 = stream.Stream()
        >>> s_1.append(Divisione('.p.'))
        >>> l = MensuralNote('A', 'longa')
        >>> s_1.append(l)
        >>> for i in range(4):
        ...     s_1.append(MensuralNote('B', 'minima'))
        >>> gmn_1 = GeneralMensuralNote('minima')
        >>> s_1.append(gmn_1)
        >>> s_1.append(MensuralNote('C', 'minima'))
        >>> s_1.append(Punctus())
        >>> gmn_1._getSurroundingMeasure(activeSite=s_1)
        ([<medren.MensuralNote minima B>,
          <medren.MensuralNote minima B>,
          <medren.MensuralNote minima B>,
          <medren.MensuralNote minima B>,
          <medren.GeneralMensuralNote minima>,
          <medren.MensuralNote minima C>], 4)

        >>> s_2 = stream.Stream()
        >>> s_2.append(Divisione('.p.'))
        >>> s_2.append(Punctus())
        >>> s_2.append(MensuralNote('A', 'semibrevis'))
        >>> s_2.append(MensuralNote('B', 'semibrevis'))
        >>> gmn_2 = GeneralMensuralNote('semibrevis')
        >>> s_2.append(gmn_2)
        >>> s_2.append(Ligature(['A', 'B']))
        >>> gmn_2._getSurroundingMeasure(activeSite=s_2)
        ([<medren.MensuralNote semibrevis A>,
          <medren.MensuralNote semibrevis B>,
          <medren.GeneralMensuralNote semibrevis>], 2)
        '''
        mOrD = mensurationOrDivisione
        if mOrD is None:
            mOrD = self._determineMensurationOrDivisione()
        ...
        for i in range(currentIndex - 1, -1, -1):
            ...
            if ((isinstance(tempList[i], notation.Punctus)) or
                    (isinstance(tempList[i], Ligature))):
                indOffset = i + 1
                break
            elif isinstance(tempList[i], GeneralMensuralNote):
                # In Italian notation, brevis, longa, and maxima indicate a new measure
                if (mOrD is not None
                        and isinstance(mOrD, notation.Divisione)
                        and tempList[i].mensuralType in ['brevis', 'longa', 'maxima']):
                    indOffset = i + 1
                    break
                else:
        ...
        for j in range(currentIndex + 1, len(tempList), 1):
            if ((isinstance(tempList[j], notation.Punctus)) or
                    (isinstance(tempList[j], Ligature))):
                break
            if isinstance(tempList[j], GeneralMensuralNote):
                if (mOrD is not None
                        and isinstance(mOrD, notation.Divisione)
                        and tempList[j].mensuralType in ['brevis', 'longa', 'maxima']):
                    break
                else:
...
class MensuralRest(GeneralMensuralNote, note.Rest):
    ...
    def __init__(self, mensuralTypeOrAbbr: str = 'brevis', **keywords):
        ...
        # GeneralMensuralNote owns the mensural-type parsing and validation:
        GeneralMensuralNote.__init__(self, mensuralTypeOrAbbr)
    ...
    def __repr__(self):
        return '<medren.MensuralRest %s>' % self.mensuralType
    ...
    def fontString(self):
        ...
        >>> from music21_tools.trecento.medren import MensuralRest
        >>> mr = MensuralRest('SB')
        >>> mr.fontString
        '0x32'
        '''
...
class MensuralNote(GeneralMensuralNote, note.Note):
    ...
    :attr:`medren.MensuralNote.pitch` and :attr:`medren.MensuralNote.mensuralType`
    respectively.
    The utf-8 code for the Ciconia font character can be accessed via the property
    :attr:`medren.MensuralNote.fontString`

    The note stems can can be set using the method :meth:`medren.MensuralNote.setStem`.
    A note's stems can be displayed using the method :meth:`medren.MensuralNote.getStems`.
    Stems may only be added to notes shorter than a brevis. For additional detail, see the
    documentation for :meth:`medren.MensuralNote.setStem`.

    The note flags can be set using the method :meth:`medren.MensuralNote.setFlag`.
    A note's flags can be displayed using the method :meth:`medren.MensuralNote.getFlags`.
    Flags may only be added to stems that exist for the given note. For additional detail,
    see the documentation for :meth:`medren.MensuralNote.setFlag`.
    ...
    def __init__(self, pitch=None, mensuralTypeOrAbbr: str = 'brevis', **keywords):
        if pitch is not None:
            note.Note.__init__(self, pitch, **keywords)  # do not replace with super
        else:
    ...
    def __repr__(self):
        return '<medren.MensuralNote %s %s>' % (self.mensuralType, self.name)

    def __eq__(self, other):
        '''
        Same as music21.GeneralNote.__eq__, but also tests equality of pitch
        and articulation.
        Only pitch is shown as a test. For other cases, please see the docs for
        :meth:``music21.GeneralMensuralNote.__eq__``

        >>> from music21_tools import trecento
        >>> from music21_tools.trecento.medren import MensuralNote
        >>> from music21_tools.trecento.notation import Divisione
        >>> from music21_tools.trecento import notation
    ...
    @GeneralMensuralNote.mensuralType.setter
    def mensuralType(self, mensuralTypeOrAbbr):
        GeneralMensuralNote.mensuralType.fset(self, mensuralTypeOrAbbr)
    ...
    def setFlag(self, stemDirection, orientation):
        ...
        >>> from music21_tools.trecento.medren import MensuralNote
        >>> r_1 = MensuralNote('A', 'minima')
        >>> r_1.setFlag('up', 'right')
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: a flag may not be added
        ...
        True
        >>> r_2 = MensuralNote('A', 'semiminima')
        >>> r_2.getFlags()
        {'up': 'right'}
        >>> r_2.setFlag('up', 'left')
        >>> r_2.getFlags()
        {'up': 'left'}
        >>> r_3 = MensuralNote('A', 'semibrevis')
        >>> r_3.setStem('side')
...
class Ligature(base.Music21Object):
    ...
    Noteheads can be also filled with the :meth:`music21.Ligature.setFillStatus` method.
    Fill status of a ligature can be determined with the
    :meth:`music21.Ligature.getFillStatus` method.

    The notes of the ligature can be accessed via the property
    :attr:`music21.Ligature.notes`.
    The mensural length of each note is calculated automatically.
    To determine if a ligature is cum proprietate, use the
    :meth:`music21.Ligature.isCumProprietate` method.
    Similarly, to determine if a ligautre is cum perfectione, use the
    :meth:`music21.Ligature.isCumPerfectione` method.
    Finally, to determine if a ligature is cum opposita proprietate (C.O.P), use the
    :meth:`music21.Ligature.isCOP` method.

    Noteheads can be set to have oblique shape using the
    :meth:`music21.Ligature.makeOblique` method. Similarly, oblique noteheads
    can be set to have a square shape using the :meth:`music21.Ligature.makeSquare` method.
    The shape of a notehead can be determined using the
    :meth:`music21.Ligature.getNoteheadShape` method. By default, all noteheads in a
    ligature are square.

    A note in the ligature can be made to be maxima by the
    :meth:`music21.Ligature.setMaxima` method. It can be determined whether a note
    is a maxima by the :meth:`music21.Ligature.isMaxima` method.
    By default, no notes in a ligature are maxima.

    A note in the ligature can be set to have an upstem, a downstem, or no
    stem by the :meth:`music21.Ligature.setStem` method.
    It can be determined whether a note has a stem by the
    :meth:`music21.Ligature.getStem` method. By default, no notes in a ligature have stems.

    A note in the ligature can be 'reversed' by the `music21.Ligature.setReverse`
    ...
    reversed by the `music21.Ligature.isReversed` method. By default, no notes
    ...
    >>> from music21_tools.trecento.medren import Ligature
    >>> l1 = Ligature(['A4', 'F4', 'G4', 'A4', 'B-4'])
    >>> l1.makeOblique(0)
    ...
    def notes(self):
        ...
        >>> print([n.mensuralType for n in l.notes])
        ['brevis', 'brevis']
        >>> l = Ligature(['G4', 'A4', 'B4', 'A4'])
        >>> l.setStem(2, 'up', 'left')
        >>> print([n.mensuralType for n in l.notes])
        ['brevis', 'brevis', 'semibrevis', 'semibrevis']
        >>> l = Ligature(['B4', 'A4', 'G4', 'A4', 'G4', 'A4', 'F4'])
        >>> l.makeOblique(0)
        >>> l.makeOblique(4)
    ...
    def makeOblique(self, startIndex):
        ...
        >>> l.makeOblique(0)
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: cannot start oblique notehead at index 0

        >>> l.makeOblique(2)
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: cannot start oblique notehead at index 2

        >>> l.makeOblique(3)
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: no note exists at index 4
        '''
    ...
    def setMaxima(self, index, value):
        ...
        >>> l.setMaxima(1, True)
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: cannot make note at index 1 a maxima

        >>> l.setMaxima(0, True)
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: cannot make note at index 0 a maxima
    ...
    def setStem(self, index, direction, orientation):
        ...
        >>> from music21_tools.trecento.medren import Ligature
        >>> l = Ligature(['A4', 'C5', 'B4', 'A4', 'B4'])
        >>> l.setStem(0, 'none', 'left')
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: direction "None" and orientation "left"
        ...
        >>> l.setStem(4, 'up', 'left')
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: cannot place stem at index 4
    ...
    def setReverse(self, endIndex, value):
        ...
        >>> l.setReverse(2, True)
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: the note at index 2
            cannot be given reverse value True

        >>> l.setReverse(3, True)
        Traceback (most recent call last):
        music21_tools.trecento.medren.MedRenException: the note at index 3
            cannot be given reverse value True
        '''
            ...
            if value in [True, False]:
                if not value:
                    ...
                    if endIndex > 0:
                        ...
                        tempPitchCurrent.accidental = None
                        tempPitchPrev.accidental = None
...
def breakMensuralStreamIntoBrevisLengths(inpStream, inpMOrD=None, printUpdates=False):
    ...
    The function :meth:`music21.breakMensuralStreamIntoBrevisLengths`
    ...
    >>> from music21_tools.trecento.medren import (
    ...     GeneralMensuralNote, MensuralNote, breakMensuralStreamIntoBrevisLengths)
    >>> from music21_tools.trecento.notation import Divisione, Punctus
    >>> s = stream.Score()
    >>> p = stream.Part()
    >>> m = stream.Measure()
    >>> s.append(p)
    >>> s.append(GeneralMensuralNote('B'))
    >>> breakMensuralStreamIntoBrevisLengths(s)
    Traceback (most recent call last):
    music21_tools.trecento.medren.MedRenException:
        cannot combine objects of type <class 'music21.stream.base.Part'>,
        <class 'music21_tools.trecento.medren.GeneralMensuralNote'> within stream
...
def setBarlineStyle(score, newStyle, oldStyle='regular', *, inPlace=False):
    ...
    for m in score.flatten(retainContainers=True):
...
def scaleDurations(score, scalingNum: int|float = 1, *, inPlace=False, scaleUnlinked=True):
    ...
    for el in score.recurse():
        ...
        if isinstance(el, tempo.MetronomeMark) and el.number is not None:
            el.number = el.number * scalingNum
...
def convertHouseStyle(score,
                      durationScale: int|float|Literal[False] = 2,
                      barlineStyle: str|Literal[False] = 'tick',
                      tieTransfer: bool = True,
                      inPlace: bool = False):
    '''
    The method :meth:`convertHouseStyle` takes a score,
    durationScale, barlineStyle, tieTransfer, and inPlace as arguments. Of these,
    only score is not optional.

    Changing :attr:`convertHouseStyle.barlineStyle`
    changes how the barlines are displayed within the piece.

    Changing :attr:`convertHouseStyle.durationScale`
    ...
    :attr:`convertHouseStyle.tieTransfer` is set to True,
    ...
    >>> from music21_tools.trecento.medren import convertHouseStyle
    ...
    >>> newGloria = convertHouseStyle(gloria, durationScale=2,
...
class TestExternal(unittest.TestCase):  # pragma: no cover
    ...
    def xtestPythagoreanSharps(self):
        from music21 import corpus, midi
        gloria = corpus.parse('luca/gloria')
        p = gloria.parts[0].flatten()
        for n in p.notes:
            if n.name == 'F#':
...
def testStretto():
    ...
    # ad te clamamus
    adTe = converter.parse('tinyNotation: 4/4 D4 F A G G F A E G F E D G C D E D G F E D')
    etJesum = converter.parse('tinyNotation: 4/4 D4 AA C D D D E E D D C G F E D G F E D C D')
