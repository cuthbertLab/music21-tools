# ------------------------------------------------------------------------------
# Name:         music21_tools/theory/mgtaPart1.py
...
'''
    # this notation excerpt is incomplete
    ex = converter.parseData(humdata)
    clefOld = ex.flatten().getElementsByClass(clef.Clef)[0]
    ex.flatten().replace(clefOld, clef.TrebleClef())
    if show:
        ex.show()
...
'''
    ...
    ex.flatten().replace(clefOld, clef.TenorClef())
...
def ch2_basic_I_A_1(show=True, *arguments, **keywords):
    ...
    ex.flatten().notesAndRests[0].addLyric('Meter: %s' % ts1)
    ex.flatten().notesAndRests[0].addLyric('Meter type: %s' % ts1.classification)
