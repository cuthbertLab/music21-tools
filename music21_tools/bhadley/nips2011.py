# ------------------------------------------------------------------------------
# Name:         music21_tools/bhadley/nips2011.py
# Purpose:      Methods used to create the data in the NeurIPS 2011 paper
#
# Authors:      Beth Hadley
...
# ------------------------------------------------------------------------------
'''
Methods used to create the data in the NeurIPS 2011 paper.

At the time of this paper the conference was called "NIPS"
(Neural Information Processing Systems), and the proceedings still appear under that name.

* Michael Scott Cuthbert, Christopher Ariza, Jose Cabal-Ugaz, Beth Hadley, and Neena Parikh.
  “Hidden Beyond MIDI’s Reach: Feature Extraction and
  Machine Learning with Rich Symbolic Formats in music21”
  Proceedings of the Neural Information Processing Systems Conference
  (NeurIPS 2011, then called NIPS) (Music and Machine Learning, Workshop 4),
'''
...
def getLeadsheetDatesFromBillboard():
    '''
    script to generate a json list matching leadsheet files to their dates.
    Compiles list of Billboard
    ...
    Both the dictionary generated by scraping the website and the json output
    has been copied into a text file and saved so this script shouldn't need to be
    re-run unless a change has been made. See text file.
    '''
    import os
    try:
...
#
#     Generates an unordered dictionary with the corresponding data: each key in
#     the dictionary is the year, each value is a list of tuples, each tuple
#     includes the song title at index 0 and group/artist at index 1.
#     The list is ordered starting at
#     element 0 by rank, starting with number 1.
    ...
    for i in dates:
        if i != 2008 and i != 2009:
            address = ('http://www.jamrockentertainment.com/'
                       'billboard-music-top-100-songs-listed-by-year/'
                       'top-100-songs-%s.html' % i)
            url = urlopen(address)
            html = url.read()
            ...
            address = ('http://www.jamrockentertainment.com/'
                       'billboard-music-top-100-songs-lististed-by-year/'
                       'top-100-songs-%s.html' % i)
            url = urlopen(address)
            html = url.read()
            soup = BeautifulSoup(html, convertEntities=BeautifulSoup.HTML_ENTITIES)
        ...
        if os.path.exists(dst):
            ...
            for year in DICT:
                    for title, _group in DICT[year]:
                        # or piece.metadata.composer == str(a):
                        if piece.metadata.movementName.lower() == str(title).lower():
                            # print("Found by title! Name",
                            #       piece.metadata.movementName,
                            #       "Composer", piece.metadata.composer, str(year))
                            # print(dst)
            ...
            if len(dates) > 0:
                ...
                # print('Title:', piece.metadata.movementName,
                #       '  Composer:', piece.metadata.composer,
                #       '  Date:', date,
                #       '  Position on Billboard:', rank)
